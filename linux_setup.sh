...
if machine_has "apt-get"; then
	sudo apt-get update \
		&& sudo apt-get install -y curl unzip python3 curl make g++ build-essential libvlc-dev vlc libx11-dev
else
	sudo yum update \
		&& sudo yum install -y curl autoconf automake bzip2 bzip2-devel cmake freetype-devel gcc gcc-c++ git libtool make pkgconfig zlib-devel libvlc-dev vlc libx11-dev
fi
